...
/*   Updated: 2026/06/08 19:30:20 by tpanou-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return (p->eof || p->line[p->line_i] == ' ' || !p->line[p->line_i]);
}

bool	go_to_next_obj(t_parsing *p)
{
	while (!p->eof && (!p->line[p->line_i] || p->line[p->line_i] == ' '))
	{
		if (!p->line[p->line_i] && go_to_next_line(p))
			return (true);
		p->line_i++;
	}
