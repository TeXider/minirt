...
{
	return (get_float(&dst->x, range, ',', p)
		|| get_float(&dst->y, range, ',', p)
		|| get_float(&dst->z, range, '\0', p));
}

bool	get_color(t_color *dst, t_parsing *p)
{
	return (get_int(&dst->r, (int [2]){0, 255}, ',', p)
		|| get_int(&dst->g, (int [2]){0, 255}, ',', p)
		|| get_int(&dst->b, (int [2]){0, 255}, '\0', p));
}
